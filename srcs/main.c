...
/*   Updated: 2025/05/09 19:04:57 by alarroye         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_child(t_token *tmp_token, char *path_cmd, t_list *env)
{
	char	**env_exec;
	...
	return (execve(path_cmd, ft_split(tmp_token->str, ' '), env_exec));
}
			...
			else if (tmp_token && tmp_token->type == WORD)
			{
					...
					&& ft_strcmp(tmp_token->str, "exit"))
				{
					if (!(ft_strchr(tmp_token->str, '/')
						...
						path_cmd = search_path(tmp_token->str,
								parse_path(data.env), &status);
					else
